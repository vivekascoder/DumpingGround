...
class SongSchema(ma.SQLAlchemySchema):
    class Meta:
        fields = ("id", "name", "singer", "lyrics")

class SongNameSchema(ma.SQLAlchemySchema):
    class Meta:
        fields = ("name", "singer")
...
song_name_schema = SongNameSchema()
# Routing Section
@app.route('/')
def index():
...
def get_lyrics():
    if request.method == "POST":
        ...
        if name != "":
            ...
            url_ = url(name)
            a = get(url_)
            temp = Song.query.filter_by(name=a['song_name']).first()
            if temp == None:
        ...
        return "Things not work like this."

@app.route('/api/songs')
def songs():
    s = Song.query.all()
    print(s)
    return jsonify(song_schema.dump(s))
